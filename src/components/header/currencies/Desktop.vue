<template>
  <div class="w-full px-10 hidden xl:flex items-center">
    ...
    <button v-for="(symbol, currency) in currencies" @click="setCurrency(currency, symbol)" :key="currency" class="flex items-center p-4 border-b-2 border-transparent hover:border-blue">
      <a href="#">{{ currency }}</a>
    </button>
  </div>
</template>
